...
/*
 * ExecScanHashBucket
 *		scan a hash bucket for matches to the current tuple in the expression context.
 *		The caller to the function must explicitly declare where the tuples to be 
 *		matched against should be placed in the expression context. The caller
 *		must also give an explicit location where to place the tuples to be 
 *		matched against ; this location is a pointer to a TupleTableSlot.
 *		
 */
 ...
 ExecScanHashBucket(HashJoinState *hjstate,
 				   ExprContext *econtext,
				   HashJoinTable hashtable,
				   bool fillOuterTupleContext,
				   TupleTableSlot *storageLocationForTuple)
 {
 	...
 	while (hashTuple != NULL)
 	{
 		if (hashTuple->hashvalue == hashvalue)
 		{
 			...
 			TupleTableSlot *tupleSlot; /****************************** CHANGED ******************/

 			/* insert hashtable's tuple into exec slot so ExecQual sees it */
 			tupleSlot = ExecStoreTuple(heapTuple,
 									  storageLocationForTuple, /****************************** CHANGED ******************/
 									  InvalidBuffer,
 									  false);	/* do not pfree */

			if (fillOuterTupleContext)
			{
...
// 									  hjstate->hj_HashTupleSlot, /******************************CHANGE******************/
// 									  InvalidBuffer,
// 									  false);	/* do not pfree */
// 			econtext->ecxt_innertuple = inntuple;    /******************************CHANGE******************/
