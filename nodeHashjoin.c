...
TupleTableSlot *
ExecFetchNextResultTupleFromEnvironnement(HashJoinState *hjstate) /* Return: a tuple or NULL */
{
	...
	if (hjstate->hj_OuterTupleProbingInner) // Context is [outer] probing [inner]
	{
		HeapTuple heapInnerTuple;
		TupleTableSlot *innerTupleSlot;
		for(;;)
		{
			/* We are probing our Outer Tuple to our Inner HashTable */
			heapInnerTuple = ExecScanHashBucket(hjstate,
																					econtext,
																					hjstate->hj_InnerHashTable,
																				  econtext->ecxt_innertuple,
																				  hj->hj_InnerTupleSlot);
	...
	} else // Context/Environnement is [inner] probing [outer]
	{
		...
		for(;;)
		{
			heapOuterTuple = ExecScanHashBucket(hjstate,
																					econtext,
																					hjstate->hj_OuterHashTable,
																				  econtext->ecxt_outertuple,
																				  hj->hj_OuterTupleSlot);
...
/********************* START NEW VERSION ********************************/
TupleTableSlot *
ExecFetchNextTupleAndEnvironnement(HashJoinState *hjstate) /* Return : a Tuple or NULL */
{

	TupleTableSlot *slot;
	PlanState outerNode = hjstate-> ---------------------;
	PlanState innerNode = hjstate-> ---------------------;
		...
		} else
		{
			if (hjstate->hj_nextFetchFromOuter) // Context will be [outer] probing [inner]
			{
				...
				if (TupIsNull(slot))
				{
					...
					// Loop back to the beggining of the fetching process to potentially find a tuple ...
				} else
				{
					hashTableToUseForProbing = hjstate->hj_innerHashTable; // We have to probe the inner relation HashTable.
					hjstate->hj_nextFetchFromOuter = innerRelationEmpty
					econtext->ecxt_outertuple = slot;
					node->hj_OuterTupleProbingInner = true;
			...
			} else // Context/Environnement will be [inner] probing [outer]
			{
				...
				} else
				{
					...
					hjstate->hj_nextFetchFromOuter = outerRelationEmpty; // Same relation if outer is empty, else context switch.
